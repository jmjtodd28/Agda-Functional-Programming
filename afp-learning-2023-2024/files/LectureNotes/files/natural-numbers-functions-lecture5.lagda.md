...
suc-is-injective : {x y : ℕ}
                 → suc x ≡ suc y
                 → x ≡ y
suc-is-injective p = ap pred p
```
...
ap-≤₀-suc : (x y : ℕ) → x ≤₀ y → suc x ≤₀ suc y
ap-≤₀-suc x y (a , p) = a , ap suc p

halb : (x y : ℕ) → x ≤₀ y → x ≤ y
halb 0       y (a , p) = 0-smallest
halb (suc x) 0 (a , ())
halb (suc x) (suc y) (a , p) = goal
 where
  b : ℕ
  b = a
...
blah : (x y : ℕ) → x ≤ y → x ≤₀ y
blah 0       y        0-smallest         = y , refl y
blah (suc x) (suc y) (suc-preserves-≤ l) = goal
 where
  IH : x ≤₀ y
  IH = blah x y l
...
example2 : 7 ≤ 11
example2 = suc-preserves-≤
             (suc-preserves-≤
              (suc-preserves-≤
               (suc-preserves-≤
                (suc-preserves-≤
                  (suc-preserves-≤
                    (suc-preserves-≤ 0-smallest))))))
